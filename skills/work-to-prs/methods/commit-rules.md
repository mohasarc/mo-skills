...
How to structure and write commits. Bundled copy of the repository rules that shaped this workflow; when the repository ships its own commit rules, read those too and let them win on conflict.
...
- Never commit on `main`. Branch `mo/<kebab-slug>` from pulled `main` first
