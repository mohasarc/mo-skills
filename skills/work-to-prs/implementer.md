...
- Read material findings carried from earlier phases. Resolve every applicable item before or alongside the assigned phase and record its disposition in the completion note.
...
Write `<pr-folder>/phase-<nn>-completion.md` using the repository completion template verbatim when present, otherwise `templates/phase-completion.md` verbatim; add a `## Step ends` list of `<step title> — <SHA>` and a `## Carried findings` disposition list at the end. Save screenshots of visual changes under `<pr-folder>/visuals/` and reference them by local path in Visuals; never commit them. Return final SHA, commits, step-end SHAs, checks, note path, carried-finding dispositions, risks, and fundamental blocker if any.
